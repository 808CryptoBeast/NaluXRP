...
async function fetchAndProcessLedger(ledgerIndex, serverInfoHint) {
  ...
  try {
    ...
    if (info) {
      s.feeAvg =
        (info.validated_ledger && info.validated_ledger.base_fee_xrp) || s.feeAvg;
      s.loadFee = (info.load_factor || 1000000) / 1000000;
...
function analyzeLedgerTransactions(ledger) {
  ...
  const avgFeeXRP =
    totalTx > 0 && totalFeeDrops > 0 ? totalFeeDrops / 1_000_000 / totalTx : 0;
...
function sendStateToDashboard() {
  ...
  // Direct call into Dashboard module (if present on window)
  ...
  // Broadcast full raw XRPL.state snapshot for analytics & others (existing)
  window.dispatchEvent(
  ...
  // ✅ Extra aliases so any dashboard build can consume state without code changes
  window.dispatchEvent(new CustomEvent("xrpl-dashboard", { detail: dashboardState }));
  window.dispatchEvent(new CustomEvent("xrpl-state", { detail: dashboardState }));
}
...
/* FIX: support BOTH navbar ids (statusDot/connectionStatus) and legacy ids (connDot/connText) */
function updateConnectionStatus(connected, label) {
  // navbar (your HTML)
  const navDot = document.getElementById("statusDot");
  const navText = document.getElementById("connectionStatus");

  // legacy / other layouts
  const dot = document.getElementById("connDot");
  const text = document.getElementById("connText");

  // helper applies to a pair if present
  function applyPair(d, t) {
    if (!d || !t) return;

    if (connected) {
      d.classList.add("active");
      d.classList.add("live");
      t.textContent = "LIVE" + (label ? " — " + label : "");
      t.style.color = "#50fa7b";
      t.style.cursor = "default";
      t.onclick = null;
    } else {
      d.classList.remove("active");
      d.classList.remove("live");
      t.textContent = label || "Connecting...";
      t.style.color = "#ffb86c";
      t.style.cursor = "pointer";
      t.title = "Click to reconnect";
      t.onclick = reconnectXRPL;
    }
  }

  applyPair(navDot, navText);
  applyPair(dot, text);
}
...
/* ---------- SHARED REQUEST WRAPPER (prevents NotConnectedError bursts) ---------- */

function waitForConnection(timeoutMs = 12000) {
  return new Promise((resolve) => {
    if (isXRPLConnected() && window.XRPL.client) return resolve(true);
    ...
    const t = setTimeout(() => {
      cleanup();
      resolve(isXRPLConnected() && !!window.XRPL.client);
    }, timeoutMs);
  ...
  for (let attempt = 0; attempt <= retries; attempt++) {
    try {
      if (!window.XRPL.client || !window.XRPL.connected) {
        const ok = await waitForConnection(Math.min(15000, timeoutMs));
        if (!ok) throw new Error("XRPL not connected");
      }
      ...
      // If we got kicked mid-request, force the normal reconnect path and retry.
      if (/NotConnected|disconnected|closed|timeout/i.test(msg)) {
        try {
          // mark as disconnected and let reconnect logic run
          window.XRPL.connected = false;
          updateConnectionStatus(false, "Disconnected");
          dispatchConnectionEvent();

          // kick reconnect (respect cooldown in connectXRPL)
          if (!window.XRPL.connecting) connectXRPL();
        } catch (_) {}
      }
...
// ✅ shared request wrapper used by inspector (if present)
