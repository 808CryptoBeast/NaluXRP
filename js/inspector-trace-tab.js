...
(function () {
  ...
  // ✅ Earliest-history scan caps (oldest-first account_tx)
  const EARLY_PAGE_LIMIT = 200;
  const EARLY_MAX_PAGES = 400;        // plenty, but bounded
  const EARLY_MAX_TX_SCAN = 200_000;  // bounded safety
  ...
  const MODULE_VERSION = "unified-inspector@3.0.1-tabs-trace-quickinspect-tokens-ws-fallback-early-history";
  ...
  async function xrplRequest(payload, { timeoutMs = 20000, allowHttpFallback = true } = {}) {
    ...
    // ✅ FIX: direct WS request can throw NotConnectedError mid-build; catch and fallback to HTTP.
    if (window.XRPL?.client?.request) {
      try {
        ...
        // nudge shared reconnect if WS dropped
        if (!computeWsConnected()) attemptSharedReconnect("ws dropped (direct_ws_client)");
  ...
  // ---------------- EARLIEST HISTORY (oldest-first account_tx) ----------------
  async function collectTxsOldestFirst(address, needCount, constraints) {
    const collected = [];
    let marker = null;
    let pages = 0;
    let scanned = 0;
    ...
    while (pages < EARLY_MAX_PAGES && scanned < EARLY_MAX_TX_SCAN) {
      pages += 1;

      const resp = await fetchAccountTxPaged(address, {
        marker,
        limit: EARLY_PAGE_LIMIT,
        forward: true,
        ledgerMin,
        ledgerMax
      });
      ...
      if (pages % 20 === 0) setStatus(`Early history ${address.slice(0, 6)}… pages:${pages} found:${collected.length}`);
    }
    ...
    return {
      txs: picked,
      meta: { pages, scanned, found: collected.length, mode: "oldest_first" }
    };
  }
  ...
  async function quickInspectAddress(address, { perNode = 120 } = {}) {
    ...
    try {
      ...
      // ✅ earliest history (opened -> early txs)
      const earlyCount = clampInt(perNode, 10, 300);
      const earlyRes = await collectTxsOldestFirst(addr, earlyCount, constraints);
      setProgress(0.52);

      // recent outgoing (most recent pages, then sorted asc for display)
      ...
      const earlyTxs = (earlyRes.txs || []).map((tx) => {
        const type = tx.TransactionType || tx.type || "Unknown";
        const from = tx.Account || tx.account || null;
        const to = tx.Destination || tx.destination || null;
        const amt = parseAmount(tx.Amount ?? tx.delivered_amount ?? tx._meta?.delivered_amount ?? null);
        return {
          tx_hash: String(tx.hash || ""),
      ...
      const metaLine = res?.meta
        ? `<div style="margin-top:10px;opacity:.8;font-size:12px;">recent outgoing scan: pages=${escapeHtml(res.meta.pages)} • scanned=${escapeHtml(
            res.meta.scanned
          )} • mode=${escapeHtml(res.meta.mode)}</div>`
        : "";

      const earlyMetaLine = earlyRes?.meta
        ? `<div style="margin-top:6px;opacity:.75;font-size:12px;">early history scan: pages=${escapeHtml(earlyRes.meta.pages)} • scanned=${escapeHtml(
            earlyRes.meta.scanned
          )} • mode=${escapeHtml(earlyRes.meta.mode)}</div>`
        : "";
      ...
      const earlyRows = earlyTxs
        .slice(0, 220)
        .map((x, i) => {
          const links = x.tx_hash ? explorerLinks(x.tx_hash) : null;
          const from = x.from ? `<code>${escapeHtml(x.from.slice(0, 10))}…</code>` : `<span style="opacity:.6;">—</span>`;
          const to = x.to ? `<code>${escapeHtml(x.to.slice(0, 10))}…</code>` : `<span style="opacity:.6;">—</span>`;
          const amt = formatAmountPretty(x.currency, x.amount, x.issuer);
          const txLink = links?.xrpscan ? `<a href="${escapeHtml(links.xrpscan)}" target="_blank" rel="noopener noreferrer">tx</a>` : "";
          return `
            <div style="padding:8px;border-bottom:1px dashed rgba(255,255,255,0.08);font-size:12px;">
              <div style="display:flex;justify-content:space-between;gap:10px;">
                <div><strong>#${i + 1}</strong> • ${escapeHtml(x.type)} • ${from} → ${to}</div>
                <div style="opacity:.8;">ledger ${escapeHtml(x.ledger_index)} • ${escapeHtml(x.date || "—")} • ${txLink}</div>
              </div>
              <div style="margin-top:4px;opacity:.9;">${escapeHtml(amt)}</div>
            </div>
      ...
      const html = `
        <div style="display:flex;gap:12px;flex-wrap:wrap;align-items:flex-start;">
          <div style="flex:1;min-width:320px;padding:12px;border-radius:12px;border:1px solid rgba(255,255,255,0.08);background:rgba(255,255,255,0.02);">
            ...
            <details style="margin-top:12px;padding:12px;border-radius:12px;border:1px solid rgba(255,255,255,0.08);background:rgba(0,0,0,0.12);">
              <summary style="cursor:pointer;font-weight:900;">Early account history (oldest-first)</summary>
              <div style="margin-top:10px;max-height:360px;overflow:auto;border-radius:10px;border:1px solid rgba(255,255,255,0.06);">
                ${earlyRows || `<div style="padding:12px;opacity:.75;">No early txs found in this range.</div>`}
              </div>
              <div style="margin-top:10px;opacity:.75;font-size:12px;">
                Note: this is an oldest-first sample (bounded). Remove date/ledger filters to see earliest activation-era activity.
              </div>
            </details>
          </div>
          ...
          <div style="width:340px;min-width:280px;padding:12px;border-radius:12px;border:1px solid rgba(255,255,255,0.08);background:rgba(255,255,255,0.02);">
            ...
            <div style="margin-top:10px;opacity:.85;font-size:12px;">
              outgoing loaded: <strong>${escapeHtml(outgoing.length)}</strong> • early loaded: <strong>${escapeHtml(earlyTxs.length)}</strong>
            </div>
            <div style="margin-top:6px;opacity:.75;font-size:12px;">
              note: issuer-tree edges exist when a counterparty can be extracted (Payment/TrustSet/OfferCreate issuer).
            </div>
          </div>
        </div>
      ...
      $("qiExportJson").onclick = () => {
        const obj = {
          ...
          earlyHistory: { txs: earlyTxs, meta: earlyRes?.meta || null },
          outgoing,
  ...
  function findPathClicked() {
    ...
    // ✅ show the edge/tx details for each hop in the path (best-effort)
    const hopDetails = [];
    for (let i = 0; i < path.length - 1; i++) {
      ...
      if (edge) {
        const links = edge.tx_hash ? explorerLinks(edge.tx_hash) : null;
        const txLink = links?.xrpscan ? `<a href="${escapeHtml(links.xrpscan)}" target="_blank" rel="noopener noreferrer">tx</a>` : "";
        const cur = edge.currency === "XRP" ? "XRP" : `${edge.currency}${edge.issuer ? ` • ${edge.issuer.slice(0, 6)}…` : ""}`;
        hopDetails.push(`
          <div style="padding:8px;border-bottom:1px dashed rgba(255,255,255,0.08);font-size:12px;">
            <div><code>${escapeHtml(a)}</code> → <code>${escapeHtml(b)}</code> • ${escapeHtml(edge.type)} <span style="opacity:.7">(${escapeHtml(edge.kind)})</span></div>
            <div style="margin-top:4px;opacity:.85;">ledger ${escapeHtml(edge.ledger_index)} • ${escapeHtml(edge.date || "—")} • ${escapeHtml(cur)} ${escapeHtml(edge.amount)} • ${txLink}</div>
          </div>
        `);
      } else {
        hopDetails.push(`
          <div style="padding:8px;border-bottom:1px dashed rgba(255,255,255,0.08);font-size:12px;opacity:.75;">
            <code>${escapeHtml(a)}</code> → <code>${escapeHtml(b)}</code> • (edge details unavailable)
          </div>
        `);
      }
    }
    ...
    $("uiResults").innerHTML = `
      ...
      <div style="margin-top:10px;">
        ${path
          .map(
            (p) => `<div style="display:flex;align-items:center;gap:8px;margin-bottom:6px;">
      ...
      <div style="margin-top:12px;padding:10px;border-radius:12px;border:1px solid rgba(255,255,255,0.06);background:rgba(255,255,255,0.02);">
        <div style="font-weight:900;">Hop transaction details (edge-backed)</div>
        <div style="margin-top:10px;max-height:260px;overflow:auto;border-radius:10px;border:1px solid rgba(255,255,255,0.06);">
          ${hopDetails.join("")}
        </div>
        <div style="margin-top:10px;opacity:.75;font-size:12px;">
          Note: hop details reflect the specific edge tx used when building the tree (not every tx between the hop pair).
        </div>
      </div>
    `;
