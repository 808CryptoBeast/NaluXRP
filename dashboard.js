...
(function () {
  ...
  const Dashboard = {
              ...
              <section class="dashboard-metrics">
                ...
                <div class="dashboard-metric-grid">
                  ...
                  <article class="metric-card">
                    ...
                    <div class="metric-sub" id="d2-quorum-note">Validators</div>
                  </article>

                  <article class="metric-card">
                    <div class="metric-label">Avg. Txn / Ledger</div>
                    <div class="metric-value" id="d2-avg-tx-per-ledger">—</div>
                    <div class="metric-sub" id="d2-tx-spread">Waiting…</div>
                  </article>

                  <article class="metric-card">
                    <div class="metric-label">Avg. Ledger Interval</div>
                    <div class="metric-value" id="d2-avg-ledger-interval">—</div>
                    <div class="metric-sub" id="d2-interval-note">Waiting…</div>
                  </article>

                  <article class="metric-card">
                    <div class="metric-label">Avg. Txn Fee</div>
                    <div class="metric-value" id="d2-avg-txn-fee">—</div>
                    <div class="metric-sub" id="d2-avg-fee-note">XRP</div>
                  </article>

                  <article class="metric-card">
                    <div class="metric-label">Load Fee</div>
                    <div class="metric-value" id="d2-load-fee">—</div>
                    <div class="metric-sub" id="d2-load-fee-note">XRP</div>
                  </article>

                  <article class="metric-card">
                    <div class="metric-label">Close Time Stability</div>
                    <div class="metric-value" id="d2-stability">—</div>
                    <div class="metric-sub" id="d2-stability-note">Waiting…</div>
                  </article>

                  <article class="metric-card">
                    <div class="metric-label">Dominant TX Type</div>
                    <div class="metric-value" id="d2-dominant-tx">—</div>
                    <div class="metric-sub" id="d2-dominant-pct">Waiting…</div>
                  </article>
                </div>
              </section>
    ...
    normalizeTxTypes(txTypes) {
      ...
      // Initialize ALL XRPL transaction types
      const agg = {
        // Core
        Payment: 0,
        TrustSet: 0,
        ...
        PermissionedDomainDelete: 0,
        
        // Multi-Purpose Tokens (MPT)
        MPTokenIssuanceCreate: 0,
        MPTokenIssuanceDestroy: 0,
        MPTokenIssuanceSet: 0,
        MPTokenAuthorize: 0,
        ...
        // XChain (Cross-Chain Bridge)
        XChainCreateBridge: 0,
        XChainModifyBridge: 0,
        XChainAccountCreateCommit: 0,
        XChainAddAccountCreateAttestation: 0,
        XChainAddClaimAttestation: 0,
        XChainClaim: 0,
        XChainCommit: 0,
        XChainCreateClaimID: 0,
        ...
        LoanBrokerSet: 0,
        LoanBrokerDelete: 0,
        LoanBrokerCoverDeposit: 0,
        LoanBrokerCoverWithdraw: 0,
        LoanBrokerCoverClawback: 0,
    ...
    updateTopMetrics(state) {
      ...
      if ($("d2-tps")) {
        ...
        $("d2-tps").textContent = tps != null ? Number(tps).toFixed(2) : "—";
      }
      if ($("d2-tps-trend")) {
      ...
      // 3. Quorum
      if ($("d2-quorum")) {
        const quorum = state.quorum || state.validatorQuorum || state.validators || state.validatorCount;
        $("d2-quorum").textContent = quorum != null ? String(quorum) : "—";
      }

      // 4. Avg. Txn/Ledger
      if ($("d2-avg-tx-per-ledger")) {
        const avgTxPerLedger = state.avgTxPerLedger || state.txPerLedger || state.txnPerLedger;
        $("d2-avg-tx-per-ledger").textContent = avgTxPerLedger != null ? Number(avgTxPerLedger).toFixed(2) : "—";
      }
      if ($("d2-tx-spread")) {
        $("d2-tx-spread").textContent = state.txSpread || "Spread: —";
      }

      // 5. Avg. Ledger Interval
      if ($("d2-avg-ledger-interval")) {
        const avgInterval = state.avgLedgerInterval || state.ledgerInterval || state.closeTimeSec;
        
        if (avgInterval != null && avgInterval > 0) {
          $("d2-avg-ledger-interval").textContent = `${avgInterval.toFixed(3)} sec`;
          
          if ($("d2-interval-note")) {
            if (avgInterval < 2.5) $("d2-interval-note").textContent = "Very Fast";
            else if (avgInterval < 3.5) $("d2-interval-note").textContent = "Fast";
            else if (avgInterval < 4.5) $("d2-interval-note").textContent = "Normal";
            else if (avgInterval < 6) $("d2-interval-note").textContent = "Slow";
            else $("d2-interval-note").textContent = "Very Slow";
          }
        } else {
          $("d2-avg-ledger-interval").textContent = "—";
          if ($("d2-interval-note")) $("d2-interval-note").textContent = "Calculating…";
        }
      }

      // 6. Avg. Txn Fee
      if ($("d2-avg-txn-fee")) {
        const avgTxnFee = state.avgTxnFee || state.avgFee || state.feeAvg;
        
        if (avgTxnFee != null) {
          $("d2-avg-txn-fee").textContent = avgTxnFee.toFixed(7);
          
          if ($("d2-avg-fee-note")) {
            const drops = Math.round(avgTxnFee * 1000000);
            $("d2-avg-fee-note").textContent = `${drops} drops`;
          }
        } else {
          $("d2-avg-txn-fee").textContent = "—";
          if ($("d2-avg-fee-note")) $("d2-avg-fee-note").textContent = "XRP";
        }
      }

      // 7. Load Fee
      if ($("d2-load-fee")) {
        const loadFee = state.loadFee || state.baseFee;
        
        if (loadFee != null) {
          $("d2-load-fee").textContent = loadFee.toFixed(5);
          
          if ($("d2-load-fee-note")) {
            const drops = Math.round(loadFee * 1000000);
            $("d2-load-fee-note").textContent = `${drops} drops`;
          }
        } else {
          $("d2-load-fee").textContent = "—";
          if ($("d2-load-fee-note")) $("d2-load-fee-note").textContent = "XRP";
        }
      }

      // 8. Close Time Stability (NEW)
      if ($("d2-stability")) {
        // Calculate standard deviation of recent ledger intervals
        const recentIntervals = this.ledgerStream
          .slice(0, 10)
          .map(l => l.closeTimeSec)
          .filter(t => t != null && t > 0);
        ...
        if (recentIntervals.length >= 3) {
          ...
          let stability;
          if (stdDev < 0.5) stability = "Stable";
          else if (stdDev < 1.0) stability = "Variable";
          else stability = "Unstable";
          
          $("d2-stability").textContent = stability;
          
          if ($("d2-stability-note")) {
            $("d2-stability-note").textContent = `±${stdDev.toFixed(2)}s`;
          }
        } else {
          $("d2-stability").textContent = "—";
          if ($("d2-stability-note")) $("d2-stability-note").textContent = "Collecting…";
        }
      }

      // 9. Dominant TX Type (NEW)
      if ($("d2-dominant-tx")) {
        const recent = this.ledgerStream.slice(0, 6);
        ...
        if (recent.length >= 3) {
          ...
          const dominancePct = totalTx > 0 ? (maxCount / totalTx) * 100 : 0;
          
          $("d2-dominant-tx").textContent = dominant;
          
          if ($("d2-dominant-pct")) {
            $("d2-dominant-pct").textContent = `${dominancePct.toFixed(1)}% dominant`;
          }
        } else {
          $("d2-dominant-tx").textContent = "—";
          if ($("d2-dominant-pct")) $("d2-dominant-pct").textContent = "Collecting…";
        }
      }
    },
    ...
    buildLedgerCardHtml(card) {
      ...
      // ✅ CHANGED: Now showing "Ledger Interval" instead of "Close Time"
      let intervalDisplay = "—";
      if (card.closeTimeSec != null) {
        const sec = card.closeTimeSec;
        if (sec < 2) intervalDisplay = `${sec.toFixed(2)}s`;
        else intervalDisplay = `${sec.toFixed(1)}s`;
      }
          ...
          <div class="ledger-card-inner">
            ...
            <div class="ledger-card-body">
              <div class="ledger-main-row">
                ...
                <div class="ledger-main-stat">
                  <span class="ledger-stat-label">Ledger Interval</span>
                  <span class="ledger-stat-value">${intervalDisplay}</span>
                </div>
                <div class="ledger-main-stat">
              ...
              <div class="ledger-type-bars">
                ...
                ${txRow("NFT Modify", t.NFTokenModify, "type-nft")}
                ...
                ${txRow("AMM Clawback", t.AMMClawback, "type-clawback")}
                ...
                ${txRow("DID Set", t.DIDSet, "type-other")}
                ${txRow("DID Delete", t.DIDDelete, "type-other")}
                
                ${txRow("Credential Create", t.CredentialCreate, "type-other")}
                ${txRow("Credential Accept", t.CredentialAccept, "type-other")}
                ${txRow("Credential Delete", t.CredentialDelete, "type-other")}
                
                ${txRow("Delegate Set", t.DelegateSet, "type-other")}
                
                ${txRow("Oracle Set", t.OracleSet, "type-other")}
                ${txRow("Oracle Delete", t.OracleDelete, "type-other")}
                
                ${txRow("PermDomain Set", t.PermissionedDomainSet, "type-other")}
                ${txRow("PermDomain Delete", t.PermissionedDomainDelete, "type-other")}
                
                ${txRow("MPT Issuance Create", t.MPTokenIssuanceCreate, "type-other")}
                ${txRow("MPT Issuance Destroy", t.MPTokenIssuanceDestroy, "type-other")}
                ${txRow("MPT Issuance Set", t.MPTokenIssuanceSet, "type-other")}
                ${txRow("MPT Authorize", t.MPTokenAuthorize, "type-other")}
                
                ${txRow("Vault Create", t.VaultCreate, "type-other")}
                ${txRow("Vault Delete", t.VaultDelete, "type-other")}
                ${txRow("Vault Deposit", t.VaultDeposit, "type-other")}
                ${txRow("Vault Withdraw", t.VaultWithdraw, "type-other")}
                ${txRow("Vault Set", t.VaultSet, "type-other")}
                ${txRow("Vault Clawback", t.VaultClawback, "type-clawback")}
                
                ${txRow("XChain CreateBridge", t.XChainCreateBridge, "type-other")}
                ${txRow("XChain ModifyBridge", t.XChainModifyBridge, "type-other")}
                ${txRow("XChain AcctCreate", t.XChainAccountCreateCommit, "type-other")}
                ${txRow("XChain AddAcctAttest", t.XChainAddAccountCreateAttestation, "type-other")}
                ${txRow("XChain AddClaimAttest", t.XChainAddClaimAttestation, "type-other")}
                ${txRow("XChain Claim", t.XChainClaim, "type-other")}
                ${txRow("XChain Commit", t.XChainCommit, "type-other")}
                ${txRow("XChain CreateClaimID", t.XChainCreateClaimID, "type-other")}
                
                ${txRow("Loan Set", t.LoanSet, "type-other")}
                ${txRow("Loan Delete", t.LoanDelete, "type-other")}
                ${txRow("Loan Manage", t.LoanManage, "type-other")}
                ${txRow("Loan Pay", t.LoanPay, "type-other")}
                ${txRow("LoanBroker Set", t.LoanBrokerSet, "type-other")}
                ${txRow("LoanBroker Delete", t.LoanBrokerDelete, "type-other")}
                ${txRow("LoanBroker CoverDeposit", t.LoanBrokerCoverDeposit, "type-other")}
                ${txRow("LoanBroker CoverWithdraw", t.LoanBrokerCoverWithdraw, "type-other")}
                ${txRow("LoanBroker CoverClawback", t.LoanBrokerCoverClawback, "type-clawback")}
                ...
                ${txRow("Batch", t.Batch, "type-other")}
                ${txRow("LedgerStateFix", t.LedgerStateFix, "type-other")}
    ...
    getDominantClass(dominantType) {
      const classMap = {
        ...
        AMMClawback: "ledger-card--amm",
        
        // Account Management
        AccountSet: "ledger-card--other",
        AccountDelete: "ledger-card--other",
        SetRegularKey: "ledger-card--other",
        SignerListSet: "ledger-card--other",
        
        // Escrow
        EscrowCreate: "ledger-card--other",
        EscrowFinish: "ledger-card--other",
        EscrowCancel: "ledger-card--other",
        
        // Payment Channels
        PaymentChannelCreate: "ledger-card--other",
        PaymentChannelFund: "ledger-card--other",
        PaymentChannelClaim: "ledger-card--other",
        
        // Checks
        CheckCreate: "ledger-card--other",
        CheckCash: "ledger-card--other",
        CheckCancel: "ledger-card--other"
      };
